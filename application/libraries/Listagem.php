...
class Listagem 
{
	...
	public function get_html()
	{
		$retorno = '';
                ...
                {
                    $retorno .= '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">'.$this->extras['grafico'].'</div>';
                }
                ...
                if( isset($this->extras['botao']) )
                {
                    $retorno .= '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">'.$this->extras['botao'].'</div>';
                }
		...
		$retorno .= '<table border="0" class="table table-striped table-advance tb_carlos">';
				...
				foreach ( $this->cabecalho as $c )
				{
					$ch = $c->chave;
                                        ...
                                        if ( isset($c->acao) && $c->acao )
                                        {
                                            if ( strstr($c->acao,'/') )
                                            {
                                                $classe = explode('/', $c->acao);
                                                if ( $classe[0] == 'imovel' )
                                                {
                                                    $model = 'imoveis_model';
                                                    $this->CI->load->model($model);
                                                    $i_ = $this->CI->{$model}->get_item_por_filtro('imoveis.id = '.$valor_);
                                                    $valor_ = isset($i_) ? $classe[1]($i_) : $valor_;
                                                }
                                                else
                                                {
                                                    $model = $classe[0].'_model';
                                                    $this->CI->load->model($model);
                                                    $i_ = $this->CI->{$model}->get_item($valor_);
                                                    $valor_ = $classe[1]($i_);
                                                }
