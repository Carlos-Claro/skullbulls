...
class Racas extends MY_Controller {
	...
	private $valida = array(
                                array( 'field'   => 'titulo',               'label'   => 'Titulo',      'rules'   => 'trim'),
                                );
    ...
    public function listar($coluna = 'id', $ordem = 'DESC', $off_set = 0)
    {
        ...
        $itens = $this->racas_model->get_itens($filtro_, $coluna, $ordem, $off_set);
        $total = $this->racas_model->get_total_itens($filtro_);
    ...
    public function exportar()
    {
        ...
        $valores = ( isset($_GET['b']) ? $_GET['b'] : array() );
        $filtro = $this->_inicia_filtros( $url, $valores );
        $data = $this->racas_model->get_itens( $filtro->get_filtro() );
    ...
    {
        $data['titulo'] = "Nova Raça";
        $id = $this->racas_model->adicionar($data);
        redirect(base_url().'racas/editar/'.$id);
    }
    ...
    public function editar($codigo = NULL, $ok = NULL){
        ...
        if ($this->form_validation->run())
        {
            ...
            if(isset($codigo))
            {
                $id = $this->racas_model->editar($data, 'racas.id = '.$codigo);
            }
            else
            {
                $id = $this->racas_model->adicionar($data);
        ...
        else
        {
                ...
                $data['tipo'] = 'Raças Editar';//$data = $this->_init_selects();
                $data['item'] = $this->racas_model->get_item($codigo);
                ...
                $this->layout
                        ...
                        ->set_include('css/estilo.css', TRUE)
                        ->set_breadscrumbs('Painel', 'painel',0)
                        ->set_breadscrumbs('Raças', 'racas', 0)
                        ->set_breadscrumbs($data['item']->titulo, 'racas', 1)
                        ->view('add_racas', $data);
        }
	...
	public function remover($id = NULL)
	{
            ...
            $quantidade = $this->racas_model->excluir('racas.id in ('.implode(',',$selecionados).')');
            if ($quantidade>0)
            {
    ...
    private function _inicia_filtros($url = '', $valores = array())
    {
        ...
        $config['itens'] = array(
            array('name' => 'id',           'titulo' => 'ID: ',     'tipo' => 'text',   'valor' => '', 'classe' => 'form-control ui-state-default', 'where' => array('tipo' => 'where', 'campo' => 'racas.id',         'valor' => '')),
            array('name' => 'titulo',   'titulo' => 'Titulo: ',   'tipo' => 'text',   'valor' => '', 'classe' => 'form-control ui-state-default', 'where' => array('tipo' => 'like', 'campo' => 'racas.titulo', 'valor' => '')),
        );
        $config['colunas'] = 2;
        $config['extras'] = '';
        $config['url'] = $url;
    ...
    private function _inicia_listagem( $itens, $extras = NULL, $exportar = FALSE )
    {
        ...
        foreach($cabecalho as $a){
            ...
            $data['operacoes'] = [
                                        (object) ['titulo' => 'Editar', 'class' => 'btn btn-info', 'icone' => '<span class="glyphicon glyphicon-pencil"></span>', 'link' => 'racas/editar/[id]'],
                                    ];

            $data['chave'] = 'id';
            $data['itens'] = $itens['itens'];
            $data['extras'] = $extras;
            $data['extras']['botao'] = '<a href="'.base_url().'racas/adicionar/" class="add-curso btn btn-success pull-right" target="_blank">Adicionar nova raça</a>';
            $data['extras']['botao'] .= '<a href="'.base_url().'racas/exportar'.$extras['get_url'].'" class="exportar btn btn-info pull-right" target="_blank">Exportar lista</a>';
