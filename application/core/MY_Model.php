...
{	
    ...
    public function __construct( $dbs = array('skullbulls') )	
    {		
        $this->db = NULL;
        parent::__construct();	
        foreach( $dbs as $db )
    ...
    {		
        $data['db'] = isset($data['db']) ? $data['db'] : 'skullbulls';
        ...
        {			
            ...
            {				
                $tipo_like = array('like','or_like','or_not_like','not_like');
                foreach ( $data['filtro'] as $f )				
                {
                    if(is_array($f) )
                    {
                        ...
                        {
                            if(array_search(strtolower($f['tipo']), $tipo_like))
                            {
                                $query->{$f['tipo']} ($f['campo'], $f['valor'],'both', ( (isset($f['unescape']) && $f['unescape']) ? FALSE : NULL ) );				
                            }
                            else
                            {
